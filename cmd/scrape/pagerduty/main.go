...
func priorityCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			dbConn, pdClient, err := newClients(c)
			if err != nil {
				return err
			}
			options := pagerduty.ScrapePrioritiesOptions{}
			logrus.Info("Beginning scrape of PagerDuty Priorities.")
			numScraped, err := pagerduty.ScrapePriorities(dbConn, pdClient, options)
			if err != nil {
				return err
			}
...
func teamCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			numScraped, err := pagerduty.ScrapeTeams(dbConn, pdClient, options)
			if err != nil {
				return err
			}
...
func escalationPolicyCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			numScraped, err := pagerduty.ScrapeEscalationPolicies(dbConn, pdClient, options)
			if err != nil {
				return err
			}
...
func serviceCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			numScraped, err := pagerduty.ScrapeServices(dbConn, pdClient, options)
			if err != nil {
				return err
			}
...
func incidentCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			numScraped, err := pagerduty.ScrapeIncidents(dbConn, pdClient, options)
			if err != nil {
				return err
			}
